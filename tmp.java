...
public class IBMModel1Aligner implements WordAligner {
	...
	private void updateTransProb(CounterMap<String, String> transProb,
		...
		for (String targetTerm : transCounts.keySet()) {
			for (String sourceTerm : transCounts.getCounter(targetTerm)
					.keySet()) {
				transProb.setCount(targetTerm, sourceTerm,
						transCounts.getCount(targetTerm, sourceTerm)/  transCounts.getCounter(targetTerm).totalCount());
			}
		}
		return;
	}
	...
	private double getAlignmentProb(int i, int j, SentencePair sentencePair,
		...
		double numerator = transProb.getCount(sentencePair.targetWords.get(i),
				sentencePair.sourceWords.get(j));
		double denormator = 0;//transProb.getCounter(sentencePair.sourceWords.get(i)).totalCount();
        for(String targetTerm : sentencePair.targetWords){
	    //cache demormator here!
            denormator += transProb.getCount(targetTerm, sentencePair.sourceWords.get(j));
        }
